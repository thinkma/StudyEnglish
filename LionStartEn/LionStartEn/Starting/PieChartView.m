...
@property (nonatomic,retain)NSMutableArray * colcorArr;
...
- (id)initWithFrame:(CGRect)frame {
 ...
 //  旋转90*c
        CGAffineTransform at =CGAffineTransformMakeRotation(6*M_PI/4);
...
- (void)toSetBoardColor:(UIColor *)color
{
    ...
    CGFloat eid = 2.0;
    self.layer.borderWidth = eid;
    self.layer.borderColor = [color CGColor];
    self.colcorArr = [NSMutableArray arrayWithObjects:color,[UIColor whiteColor], nil];
...
- (void)drawRect:(CGRect)rect {
    ...
    NSInteger wedges = [self.valueArr count];
    if (wedges > [self.colcorArr count]) {
        NSLog(@"Number of colors is not enough: please add %u kinds of colors.",wedges - [self.colcorArr count]);
        for (NSInteger i= [self.colcorArr count]; i<wedges; ++i) {
            [self.colcorArr addObject:[UIColor whiteColor]];
        }
    }
    ...
    for (int i = 0; i < wedges; ++i) {
        sum += [[self.valueArr objectAtIndex:i] floatValue];
    }
    ...
    for (int i = 0; i < wedges; ++i) {
        startAngle = endAngle;
        endAngle  += [[self.valueArr objectAtIndex:i] floatValue] * frac;
        [mThetaArray addObject:[NSNumber numberWithFloat:endAngle]];
        [[self.colcorArr objectAtIndex:i] setFill];
...
- (void)tosetValue:(CGFloat)value
{
    ...
    CGFloat otherVlaue = 10 - value;
    
    self.valueArr = [NSMutableArray arrayWithObjects:[NSNumber numberWithInt:value],
    [NSNumber numberWithInt:otherVlaue],nil];
